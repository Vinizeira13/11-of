...
import { ArrowRight, ChevronRight, Clock, Package, Truck } from "lucide-react";
...
export default async function AdminPedidosPage() {
  ...
  // Priority list: paid & not dispatched goes first (actionable)
  ...
  const waiting = orders.filter((o) => o.payment_status !== "completed");
  const done = orders.filter(
    (o) => o.delivery_status === "delivered" || o.delivery_status === "returned",
  );
    ...
    <section className={cn("mb-10", muted && "opacity-80")}>
      ...
      {rows.length === 0 ? (
        ...
        <ul className="divide-y divide-border/60 rounded-xl border border-border/60 bg-card/30">
          {rows.map((o) => {
            const pill = STATUS_PILLS[o.delivery_status] ?? STATUS_PILLS.pending;
            return (
              <li key={o.id}>
                <Link
